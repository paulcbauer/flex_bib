...
`flex_bib` can solve those problems automatically for us namely to tidy up (delete unused references from) a larger bib file based on the citations in a .rmd file. It creates a new cleaned bib file(s). `flex_bib` can also rely on several bib files as input and merge them.


## Installation
The function `flex_bib_file` is currently not part of a package. So simply run the code below to source the function from github. You will need the `df2bib` package.
...
`pandoc-citeproc`.
  ...
  - Download the file `multiple-bibliographies.lua` and store it as `multiple-bibliographies.lua` (not as .txt file) in the same folder as your Rmarkdown file.

## How to

Information on how to use the function can be found in the vignette: 

* PDF: https://github.com/paulcbauer/flex_bib/blob/master/vignette.pdf ([Download link](https://github.com/paulcbauer/flex_bib/raw/master/vignette.pdf))
