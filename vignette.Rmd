...
### What if I have 1 (or $geq$ 3) input `.bib` files?

For a single `.bib` files, simply specify `bib_input = "path/to/my/bibfile.bib"`. For any number of `.bib` files, collected the path names and/or file names in a vector, e.g. `bib_input = c("path/to/my/first/bibfile1.bib", "path/to/my/other/bibfile2.bib", "path/to/my/last/bibfile3.bib")`.

### In a document with many sections/chapters, how can I include separate bibliography for each of them? 
...
1. the enumerated `bib_output` files that `tidy_bib()` will produce;
...
Here is the rough structure of an `.Rmd` file for a thesis with five chapters with chapter-specific bibliographies, using a single large input `.bib` file. Not that we only need to specify four anchors to obtain five separate bibliographies:
